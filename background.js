...
function getSlackUrl() {
  return "https://"+localStorage.slackName+".slack.com";
}
function getSlackChInfoUrl() {
  return "https://slack.com/api/channels.info?token="+localStorage.slackApiToken;
}
function getSlackChListUrl() {
  return "https://slack.com/api/channels.list?token="+localStorage.slackApiToken;
}


function isSlackUrl(url) {
  // Return whether the URL starts with the slack domain prefix.
  return url.indexOf(getSlackUrl()) == 0;
}
...
function updateIcon() {
  if (!localStorage.hasOwnProperty('unreadCount')) {
	...
	if (localStorage.counterDisplay == 0) {
		...
		var nb_messages = parseInt(localStorage.unreadCount)
...
function startRequest(params) {
  ...
  getChannelList(
	  function(list) {
		  localStorage.totalUnread = 0;
		  for( i=0; i<list.length-1; ++i ) { // N-1 channels
			  var chName = list[i].name
			  var chId = list[i].id
			  getUnreadCount(
				  function(count, name) {
					  localStorage.totalUnread = parseInt(localStorage.totalUnread)+parseInt(count);
					  console.log(count+' unread messages in channel '+name+'\n');
				  },
				  function() {
					  console.log("error in getUnreadCount");
		  ...
		  var chName = list[list.length-1].name;
		  var chId = list[list.length-1].id;
		  // last channel
		  getUnreadCount(
			  function(count, name) {
				  localStorage.totalUnread = parseInt(localStorage.totalUnread)+parseInt(count);
	  ...
	  function () {
		  console.log('error in getChannelList');
		  stopLoadingAnimation();
		  delete localStorage.unreadCount;
		  updateIcon();
	  }
...
function getChannelList(onSuccess, onError) {
  ...
  try {
    ...
    xhr.open("GET", getSlackChListUrl(), true);
    xhr.send(null);
  } catch(e) {
...
//The acutal function to get the number of message
function getUnreadCount(onSuccess, onError, channel, name) {
  var xhr = new XMLHttpRequest();
  var abortTimerId = window.setTimeout(function() {
    console.warn('timeout as fuck');
	xhr.abort();  // synchronously calls onreadystatechange
  ...
  try {
    xhr.onreadystatechange = function() {
      ...
      if (xhr.responseText) {
		var jsonResponse = JSON.parse(xhr.responseText);
		var message = jsonResponse.channel.unread_count_display;
    ...
    xhr.open("GET", getSlackChInfoUrl()+"&channel="+channel, true);
	
    xhr.send(null);
  } catch(e) {
    console.error(chrome.i18n.getMessage("slackcheck_exception", e));
    handleError();
  }
}
...
function updateUnreadCount() {
  count = localStorage.totalUnread
...
function goToSlack() {
  console.log('Going to slack.');
  chrome.tabs.getAllInWindow(undefined, function(tabs) {
    for (var i = 0, tab; tab = tabs[i]; i++) {
      if (tab.url && isSlackUrl(tab.url)) {
        console.log('Found Slack tab: ' + tab.url + '. ' +
    ...
    console.log('Could not find Slack tab. Creating one...');
    chrome.tabs.create({url: getSlackUrl()});
  });
}
...
function setOptions() {
	//Set default icon
	setNoOveride('disconnected_img','icon_19.png');
	setNoOveride('no_message_img','icon_19.png');
	setNoOveride('message_img','icon_19.png');
...
var filters = {
  ...
  url: [{urlContains: getSlackUrl().replace(/^https?\:\/\//, '')}]
};

function onNavigate(details) {
  if (details.url && isSlackUrl(details.url)) {
    console.log('Recognized slack navigation to: ' + details.url + '.' +
                'Refreshing count...');
    startRequest({scheduleRequest:false, showLoadingAnimation:false});
  }
}
...
chrome.browserAction.onClicked.addListener(goToSlack);
